...
import pandas as pd
from .file_db import load_contacts, save_contacts, make_contact_id
...
def display_contact(contact):
    for contact in contacts:
        with st.expander(contact['name']):
            ...
            st.write(f"Сімейний стан: {contact.get('family')}")
...
def get_contact_form_data(contacts):
    ...
    familys = ('Одружен(а)/ий', 'Не одружен(а)/ий')

    with st.form("contact_form"):
        st.subheader("Новий контакти")
        ...
        family = st.selectbox(f"Сімейний стан:", familys)
    ...
    if submitted:
        ...
        if not name:
            return False, "І'мя не нає бути порожнім.", {}
        if not phone_ok:
...
def filter_contacts(contacts, search_text, selected_city):
    ...
    for contact in contacts:
        ...
        matches_city = (contact.get('city', '') == selected_city or selected_city == 'Усі міста')
...
def show_contacts_page():
    st.title('Сисок контактів')

    contacts = load_contacts()

    list_tab, add_tab, del_tab = st.tabs(["Мої контакти", "Додати контакти", "Видалити контакт"])
    ...
    with del_tab:
        ...
        if contacts:
            ...
            contact = st.selectbox("Контакти", contacts,
            ...
            st.info("Покищо немає контактів для видалення.")
