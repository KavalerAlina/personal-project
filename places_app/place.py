    ...
    def to_html(self):
        return f"""
            <article class="places-card">
                <img class="places-image" src="{self.image_url}" alt="{self.name}" />
        
                <div class="place-content">
                    <p class="country">{self.country}</p>
                    <h2>{self.name}</h2>
                    <p class="discription">{self.description}</p>
                    <span class="status {self.get_status_class()}">{self.get_status()}</span>
                </div>
            </article>
        """